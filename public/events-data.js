const SERVER_UTC_OFFSET = 2;

const EVENTS_DATA = {
    events: [
        {
            id: 'blood-castle',
            name: 'Blood Castle',
            icon: '🏰',
            img: 'https://muonlinefanz.com/tools/npc/data/graphics/Messenger%20Of%20Archangel/Messenger%20Of%20Archangel.jpg',
            color: '#c62828',
            category: 'events',
            description: { pt: 'Evento PvE - Mate monstros, destrua o portão e entregue a Arma do Arcanjo', en: 'PvE Event - Kill monsters, destroy the gate and deliver the Archangel Weapon' },
            times: ['00:30', '01:30', '02:30', '03:30', '04:30', '05:30', '06:30', '07:30', '08:30', '09:30', '10:30', '11:30', '12:30', '13:30', '14:30', '15:30', '16:30', '17:30', '18:30', '19:30', '20:30', '21:30', '22:30', '23:30'],
            duration: 21,
            gateClose: 5
        },
        {
            id: 'devil-square',
            name: 'Devil Square',
            icon: '👹',
            img: 'https://muonlinefanz.com/tools/npc/data/graphics/Charon/Charon.jpg',
            color: '#6a1b9a',
            category: 'events',
            description: { pt: 'Evento PvE - Ondas de monstros para ganhar experiência e Zen', en: 'PvE Event - Monster waves to earn experience and Zen' },
            times: ['00:00', '01:00', '02:00', '03:00', '04:00', '05:00', '06:00', '07:00', '08:00', '09:00', '10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00', '17:00', '18:00', '19:00', '20:00', '21:00', '22:00', '23:00'],
            duration: 26,
            gateClose: 5
        },
        {
            id: 'chaos-castle',
            name: 'Chaos Castle',
            icon: '⚔️',
            img: 'https://muonlinefanz.com/tools/npc/data/graphics/Gallus%20The%20Elder/Gallus%20The%20Elder.jpg',
            color: '#e65100',
            category: 'events',
            description: { pt: 'Evento PvP - Lute contra jogadores e monstros, último sobrevivente vence', en: 'PvP Event - Fight players and monsters, last survivor wins' },
            times: ['01:55', '05:55', '09:55', '13:55', '17:55', '21:55'],
            duration: 16,
            gateClose: 5
        },
        {
            id: 'colossus-invasion',
            name: 'Invasão do Colossus',
            icon: '🛡️',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/383/body/wc5-Xa7j3BKEdMrXv-nwX.gif',
            color: '#00838f',
            category: 'events',
            description: { pt: 'PvPvE - Derrote Colossus Guardians em Ruined Devias para Jewel of Guardian', en: 'PvPvE - Defeat Colossus Guardians in Ruined Devias for Jewel of Guardian' },
            times: ['13:00', '17:00', '21:00'],
            duration: 20
        },
        {
            id: 'battle-royale',
            name: 'Batalha Real',
            icon: '🎯',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/49/body/hAXpnjHzyQ7bZGCIh4ZEz_334X450.webp',
            color: '#1565c0',
            category: 'events',
            description: { pt: 'PvP 50vs50 - Times lutam com equipamento inicial, colete loot para vencer', en: 'PvP 50vs50 - Teams fight with starter gear, collect loot to win' },
            times: ['02:00', '08:00', '14:00', '20:00', '23:00'],
            duration: 10,
            earlyWarning: 7
        },
        {
            id: 'golden-invasion',
            name: 'Golden Invasion',
            icon: '✨',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/112/body/asngWVm4_UTe722n8dYCF.gif',
            color: '#f9a825',
            category: 'events',
            description: { pt: 'Monstros dourados aparecem em vários mapas com drops valiosos', en: 'Golden monsters spawn across multiple maps with valuable drops' },
            times: ['00:31', '04:31', '08:31', '12:31', '16:31', '20:31'],
            duration: 20
        },
        {
            id: 'castle-siege',
            name: 'Castle Siege',
            icon: '🏯',
            img: 'https://muonlinefanz.com/tools/npc/data/graphics/Guardsman/Guardsman.jpg',
            color: '#37474f',
            category: 'events',
            description: { pt: 'Evento semanal de guilds - Domingo às 20:00 (horário servidor)', en: 'Weekly guild event - Sunday at 20:00 (server time)' },
            times: ['20:00'],
            duration: 60,
            weekday: 0
        }
    ],

    bosses: [
        {
            id: 'white-wizard',
            name: 'White Wizard',
            icon: '🧙',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/125/body/7NJ6PW2Az1QucmJeMCXyN.gif',
            color: '#e0e0e0',
            category: 'bosses',
            description: { pt: 'Localização: Noria', en: 'Location: Noria' },
            times: ['09:45', '12:45', '15:45', '18:45'],
            duration: 0
        },
        {
            id: 'death-king',
            name: 'Death King',
            icon: '💀',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/126/body/-GwVeKpTYRMlJCyC5IAP0.gif',
            color: '#b71c1c',
            category: 'bosses',
            description: { pt: 'Localização: Shadow Abyss', en: 'Location: Shadow Abyss' },
            times: ['21:45', '00:45', '03:45', '06:45'],
            duration: 0
        },
        {
            id: 'zaikan',
            name: 'Zaikan',
            icon: '🗡️',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/127/body/CzlHzu6gl8HQxTMl2zzSi.gif',
            color: '#4a148c',
            category: 'bosses',
            description: { pt: 'Localização: Tarkan 2 (3 spawns)', en: 'Location: Tarkan 2 (3 spawns)' },
            times: ['00:55', '06:55', '12:55', '18:55'],
            duration: 0
        },
        {
            id: 'red-dragon',
            name: 'Red Dragon',
            icon: '🐉',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/128/body/9glyr72i5m_9nR73DihnY.gif',
            color: '#d32f2f',
            category: 'bosses',
            description: { pt: 'Localização: Aida (3 spawns)', en: 'Location: Aida (3 spawns)' },
            times: ['08:00', '20:00'],
            duration: 0
        },
        {
            id: 'cursed-santa',
            name: 'Cursed Santa',
            icon: '🎅',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/129/body/KuggfdVhYaPT8nQe7yvJs.gif',
            color: '#c62828',
            category: 'bosses',
            description: { pt: 'Localização: Devias', en: 'Location: Devias' },
            times: ['02:35', '08:35', '14:35', '20:35'],
            duration: 0
        },
        {
            id: 'death-bone',
            name: 'Death Bone',
            icon: '☠️',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/233/body/pYcl1Dt9bjQ7eyrjKSNPv.gif',
            color: '#424242',
            category: 'bosses',
            description: { pt: 'Localização: Shadow Abyss (10 spawns)', en: 'Location: Shadow Abyss (10 spawns)' },
            times: ['21:45', '00:45', '03:45', '06:45'],
            duration: 0
        },
        {
            id: 'cursed-goblin',
            name: 'Cursed Goblin',
            icon: '👺',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/133/body/PrtRqamLz3NqKBygb-INq.gif',
            color: '#2e7d32',
            category: 'bosses',
            description: { pt: 'Localização: Devias (10 spawns)', en: 'Location: Devias (10 spawns)' },
            times: ['02:35', '08:35', '14:35', '20:35'],
            duration: 0
        },
        {
            id: 'destructive-ogre',
            name: 'Destructive Ogre (Archer & Soldier)',
            icon: '👹',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/234/body/aLiWVJHTzWqkoFVErIQpo.gif',
            color: '#5d4037',
            category: 'bosses',
            description: { pt: 'Localização: Noria (5 spawns cada)', en: 'Location: Noria (5 spawns each)' },
            times: ['09:45', '12:45', '15:45', '18:45'],
            duration: 0
        },
        {
            id: 'frigidon',
            name: 'Frigidon',
            icon: '🥶',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/569/body/9DpdTbEAjju_xtoCDX8-k.gif',
            color: '#0288d1',
            category: 'bosses',
            description: { pt: 'Localização: Ruined Devias (3 spawns) - Dropa pedras para Guild Shrine', en: 'Location: Ruined Devias (3 spawns) - Drops stones for Guild Shrine' },
            times: ['00:00', '03:00', '06:00', '09:00', '12:00', '15:00', '18:00', '21:00'],
            duration: 0
        },
        {
            id: 'cryonox',
            name: 'Cryonox',
            icon: '🐲',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/568/body/1Cl0MlICu9U6D7oXO2BIa.gif',
            color: '#d32f2f',
            category: 'bosses',
            description: { pt: 'Localização: Twisted Karutan (4) + Land of Trials (2) — 6 spawns · Dropa pedra aleatória para Guild Shrine · ⚠️ Apenas no servidor principal', en: 'Location: Twisted Karutan (4) + Land of Trials (2) — 6 spawns · Drops random stone for Guild Shrine · ⚠️ Main server only' },
            times: [],
            duration: 0,
            respawnInfo: { pt: '5-7 horas após a morte', en: '5-7 hours after death' },
            subServers: [1, 2, 3],
            spawnsPerServer: 6,
            respawnMinHours: 5,
            respawnMaxHours: 7
        },
        {
            id: 'skeleton-king',
            name: 'Skeleton King',
            icon: '👑',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/671/body/jtt9Lf5KV1tR-crKqoCnh.gif',
            color: '#4e342e',
            category: 'bosses',
            description: { pt: 'Localização: Ruined Lorencia, Twisted Karutan, Shadow Abyss', en: 'Location: Ruined Lorencia, Twisted Karutan, Shadow Abyss' },
            times: ['03:45', '09:45', '15:45', '21:45'],
            duration: 0
        },
        {
            id: 'lord-of-kundun',
            name: 'Lord of Kundun',
            icon: '👹',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/218/body/JalNb7K9ipEOikEljDVQZ.gif',
            color: '#311b92',
            category: 'bosses',
            description: { pt: 'Localização: Shadow Abyss - Drop: 2-3 Armas, 1-2 Ancient T4 (7.3)', en: 'Location: Shadow Abyss - Drop: 2-3 Weapons, 1-2 Ancient T4 (7.3)' },
            times: ['01:50', '15:50'],
            duration: 0
        },
        {
            id: 'muggron',
            name: 'Muggron',
            icon: '🦇',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/455/body/-4Ydi-xNXdkqPiSqscZFg.gif',
            color: '#880e4f',
            category: 'bosses',
            description: { pt: 'Localização: 2x Shadow Abyss + 2x Crywolf + 2x Barracks of Balgass (6 spawns)', en: 'Location: 2x Shadow Abyss + 2x Crywolf + 2x Barracks of Balgass (6 spawns)' },
            times: [],
            duration: 0,
            respawnInfo: { pt: '3-4 horas após a morte (patch 7.3)', en: '3-4 hours after death (patch 7.3)' }
        },
        {
            id: 'abbadon',
            name: 'Abbadon',
            icon: '😈',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/399/body/xVJkXvAbOOCXcfqdo83VN.gif',
            color: '#b71c1c',
            category: 'bosses',
            description: { pt: 'Localização: Twisted Karutan - Drop: 2 Weapons, 4 Armors', en: 'Location: Twisted Karutan - Drop: 2 Weapons, 4 Armors' },
            times: ['03:50', '17:50'],
            duration: 0
        },
        {
            id: 'infernal-overlord',
            name: 'Infernal Overlord',
            icon: '🔥',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/96/body/PFnAU0WZhu0BwBLZ40cvP.gif',
            color: '#ff5722',
            category: 'bosses',
            description: { pt: 'Localização: Kanturu Labyrinth - Drop: 5-7 Items', en: 'Location: Kanturu Labyrinth - Drop: 5-7 Items' },
            times: ['04:50', '20:50'],
            duration: 0
        },
        {
            id: 'aurindra',
            name: 'Aurindra',
            icon: '🦅',
            img: 'https://dreamassets.fra1.cdn.digitaloceanspaces.com/images/567/body/RGp2BeDE0ATGUnrH5PR9c.gif',
            color: '#7e57c2',
            category: 'bosses',
            description: { pt: 'Localização: Crimson Icarus - Drop: Wing lvl 2, Phoenix Feather, Wing lvl 2.5', en: 'Location: Crimson Icarus - Drop: Wing lvl 2, Phoenix Feather, Wing lvl 2.5' },
            times: ['23:50'],
            duration: 0
        }
    ],

    elites: [
        {
            id: 'nethrakar',
            name: 'Nethrakar',
            icon: '💀',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/662/body/yWulQ5S3n-ec6iXEdrccv.gif',
            color: '#8d6e63',
            category: 'elites',
            map: 'Noria',
            description: 'Noria',
            times: [],
            duration: 0,
            respawnMinutes: 60,
            respawnInfo: { pt: '1 hora após a morte', en: '1 hour after death' }
        },
        {
            id: 'crystal-warden',
            name: 'Crystal Warden',
            icon: '🔮',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/663/body/sosa2ZUMch7GNZ3Xg0YK6.gif',
            color: '#4fc3f7',
            category: 'elites',
            map: 'Devias',
            description: 'Devias',
            times: [],
            duration: 0,
            respawnMinutes: 60,
            respawnInfo: { pt: '1 hora após a morte', en: '1 hour after death' }
        },
        {
            id: 'mordrax',
            name: 'Mordrax',
            icon: '🦎',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/664/body/x6ALiVunTInzXDKlWtYrP.gif',
            color: '#66bb6a',
            category: 'elites',
            map: 'Dream Land',
            description: 'Dream Land',
            times: [],
            duration: 0,
            respawnMinutes: 60,
            respawnInfo: { pt: '1 hora após a morte', en: '1 hour after death' }
        },
        {
            id: 'ember-golem',
            name: 'Ember Golem',
            icon: '🪨',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/665/body/0MoGHfTnsChnaYLjiWlo_.gif',
            color: '#ff7043',
            category: 'elites',
            map: 'Losttower',
            description: 'Losttower',
            times: [],
            duration: 0,
            respawnMinutes: 60,
            respawnInfo: { pt: '1 hora após a morte', en: '1 hour after death' }
        },
        {
            id: 'arcane-sentinel',
            name: 'Arcane Sentinel',
            icon: '🛡️',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/666/body/ZVcSXx1R3yHLlEnvSbTVf.gif',
            color: '#ef5350',
            category: 'elites',
            map: 'Elveland',
            description: 'Elveland',
            times: [],
            duration: 0,
            respawnMinutes: 60,
            respawnInfo: { pt: '1 hora após a morte', en: '1 hour after death' }
        },
        {
            id: 'hellcarion',
            name: 'Hellcarion',
            icon: '😈',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/668/body/nDNsnkmFRgMgoHsGg6JBO.gif',
            color: '#7e57c2',
            category: 'elites',
            map: 'Shadow Abyss',
            description: 'Shadow Abyss',
            times: [],
            duration: 0,
            respawnMinutes: 60,
            respawnInfo: { pt: '1 hora após a morte', en: '1 hour after death' }
        },
        {
            id: 'tidebreaker',
            name: 'Tidebreaker',
            icon: '🌊',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/669/body/aeB06DAzVtHDHCVDcNteM.gif',
            color: '#5c6bc0',
            category: 'elites',
            map: 'Atlans',
            description: 'Atlans',
            times: [],
            duration: 0,
            respawnMinutes: 60,
            respawnInfo: { pt: '1 hora após a morte', en: '1 hour after death' }
        },
        {
            id: 'mines-of-guardian',
            name: 'Mines of Guardian',
            icon: '⛏️',
            color: '#00bcd4',
            category: 'elites',
            map: 'Ruined Devias',
            description: 'Ruined Devias (10 spawns) - Dropa Jewel of Guardian',
            times: [],
            duration: 0,
            respawnMinutes: 60,
            respawnInfo: { pt: '45-75 minutos após a morte', en: '45-75 minutes after death' }
        },
        {
            id: 'moltragon',
            name: 'Moltragon',
            icon: '🐲',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/537/body/BeUF4rm4Zkp_ncOAwiPpc.gif',
            color: '#6a1b9a',
            category: 'elites',
            map: 'Shadow Abyss',
            description: 'Shadow Abyss (2 spawns) - Dropa Escudos',
            times: [],
            duration: 0,
            respawnMinutes: 90,
            respawnInfo: { pt: '1-2 horas após a morte', en: '1-2 hours after death' }
        },
        {
            id: 'dreadhorn',
            name: 'Dreadhorn',
            icon: '🦬',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/536/body/7iYCe5aEt9H8YtvRagaWt.gif',
            color: '#5d4037',
            category: 'elites',
            map: 'Shadow Abyss',
            description: 'Shadow Abyss (2 spawns) - Dropa Escudos',
            times: [],
            duration: 0,
            respawnMinutes: 90,
            respawnInfo: { pt: '1-2 horas após a morte', en: '1-2 hours after death' }
        },
        {
            id: 'kharzul',
            name: 'Kharzul',
            icon: '👺',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/453/body/Bbx9qKp9RyqcsLosn5l45.gif',
            color: '#4a148c',
            category: 'elites',
            map: 'Shadow Abyss',
            description: 'Shadow Abyss (1 spawn) - Dropa Ancient T4',
            times: [],
            duration: 0,
            respawnMinutes: 450,
            respawnInfo: { pt: '7-8 horas após a morte', en: '7-8 hours after death' }
        },
        {
            id: 'vescrya',
            name: 'Vescrya',
            icon: '🦂',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/454/body/fNaLVB03_Yag-EEYo0rTu.gif',
            color: '#1b5e20',
            category: 'elites',
            map: 'Shadow Abyss',
            description: 'Shadow Abyss (1 spawn) - Dropa Ancient T4',
            times: [],
            duration: 0,
            respawnMinutes: 450,
            respawnInfo: { pt: '7-8 horas após a morte', en: '7-8 hours after death' }
        },
        {
            id: 'borgar',
            name: 'Borgar',
            icon: '⚔️',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/535/body/yGv23akxapPHZkYC0I452.gif',
            color: '#bf360c',
            category: 'elites',
            map: 'Shadow Abyss',
            description: 'Shadow Abyss (1 spawn) - Dropa Armas',
            times: [],
            duration: 0,
            respawnMinutes: 150,
            respawnInfo: { pt: '2-3 horas após a morte', en: '2-3 hours after death' }
        }
    ],

    cherry: [
        {
            id: 'lunar-rabbit',
            name: 'Lunar Rabbit',
            icon: '🐰',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/473/body/d9uc1PDeQyp6-TE7bv8CO.gif',
            color: '#ec407a',
            category: 'cherry',
            description: { pt: 'Dreamland - Colete 10 galhos para trocar no NPC', en: 'Dreamland - Collect 10 branches to exchange at NPC' },
            times: ['05:25', '11:25', '17:25', '23:25'],
            duration: 0
        },
        {
            id: 'fire-flame-ghost',
            name: 'Fire Flame Ghost',
            icon: '🔥',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/474/body/MFf2VGiq_jQn1AqG8rl9C.gif',
            color: '#ff5722',
            category: 'cherry',
            description: { pt: 'Dreamland - Colete 30 galhos para trocar no NPC', en: 'Dreamland - Collect 30 branches to exchange at NPC' },
            times: ['01:25', '07:25', '13:25', '19:25'],
            duration: 0
        },
        {
            id: 'pouch-of-blessing',
            name: 'Pouch of Blessing',
            icon: '🎁',
            img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/475/body/DpTtcEAozg3MdmQa1TNSL.gif',
            color: '#ec407a',
            category: 'cherry',
            description: { pt: 'Dreamland (50 spawns) - Colete 10 e troque no NPC Cherry Blossom', en: 'Dreamland (50 spawns) - Collect 10 and exchange at NPC Cherry Blossom' },
            times: ['03:25', '09:25', '15:25', '21:25'],
            duration: 0
        }
    ],

    // Golden Invasion monsters — spawn during 00:36 / 04:36 / 08:36 / 12:36 / 16:36 / 20:36
    // Source: https://mudream.online/about/monsters/golden-monsters
    golden: [
        // Noria
        { id: 'g-goblin',       name: 'Golden Goblin',        map: 'Noria',         qty: 5, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/112/body/asngWVm4_UTe722n8dYCF.gif' },
        { id: 'g-golem',        name: 'Golden Golem',         map: 'Noria',         qty: 3, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/223/body/ur5z2GTQ-L8AV3jGKid5v.gif' },
        // Dream Land
        { id: 'g-budgedragon',  name: 'Golden Budge Dragon',  map: 'Dream Land',    qty: 5, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/107/body/haCnfHpnvEaGzWJ9fbbPA.gif' },
        { id: 'g-giant',        name: 'Golden Giant',         map: 'Dream Land',    qty: 3, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/225/body/uH1ayMdUEifF3JwU_BXKn.gif' },
        // Elbeland
        { id: 'g-rabbit',       name: 'Golden Rabbit',        map: 'Elbeland',      qty: 5, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/236/body/ZFM3E7_lOGVLWTJF75NNO.gif' },
        { id: 'g-grizzly',      name: 'Golden Grizzly',       map: 'Elbeland',      qty: 3, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/226/body/KQyGSatPSFByrLr6v9P0W.gif' },
        // Devias
        { id: 'g-eliteyeti',    name: 'Golden Elite Yeti',    map: 'Devias',        qty: 4, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/227/body/-nSyuuK5T0WxUthTVwZjE.gif' },
        { id: 'g-soldier',      name: 'Golden Soldier',       map: 'Devias',        qty: 3, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/108/body/pBs0mmXeg2vt40LUlTs_d.gif' },
        // Shadow Abyss
        { id: 'g-gorgon',       name: 'Golden Gorgon',        map: 'Shadow Abyss',  qty: 4, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/228/body/_BEYyx0hhXw9zP8OKIr4V.gif' },
        { id: 'g-darkknight',   name: 'Golden Dark Knight',   map: 'Shadow Abyss',  qty: 3, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/110/body/FTtFAzasa3_Kn6x0buukS.gif' },
        // Atlans 1-2
        { id: 'g-vepar',        name: 'Golden Vepar',         map: 'Atlans 1 - 2',  qty: 4, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/115/body/vSuJBsKlDj62ZWWrFTbjy.gif' },
        { id: 'g-valkyrie',     name: 'Golden Valkyrie',      map: 'Atlans 1 - 2',  qty: 3, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/229/body/H9Zd5SnrgaR7j7z7Q9OFT.gif' },
        // Atlans 2-3
        { id: 'g-titan',        name: 'Golden Titan',         map: 'Atlans 2 - 3',  qty: 3, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/111/body/HxYhQA42j2Lkjhi8OKC6p.gif' },
        { id: 'g-lizardking',   name: 'Golden Lizard King',   map: 'Atlans 2 - 3',  qty: 2, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/114/body/__Yi_hb0A9fW6nQVhxFzO.gif' },
        // Losttower 1-7
        { id: 'g-devil',        name: 'Golden Devil',         map: 'Losttower 1 - 7', qty: 3, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/113/body/3CciBn7d2BorA1d9pz_D9.gif' },
        { id: 'g-balrog',       name: 'Golden Balrog',        map: 'Losttower 1 - 7', qty: 2, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/230/body/AzPmnnwQZCqiPezxAMVvm.gif' },
        // Tarkan 1
        { id: 'g-wheel',        name: 'Golden Wheel',         map: 'Tarkan 1',      qty: 3, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/117/body/TsaHRUcGxMvnUe0cVQDxd.gif' },
        { id: 'g-mutant',       name: 'Golden Mutant',        map: 'Tarkan 1',      qty: 2, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/246/body/eqTrkwY1F5uDfY5EWKiLH.gif' },
        // Tarkan 2
        { id: 'g-tantalos',     name: 'Golden Tantalos',      map: 'Tarkan 2',      qty: 2, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/116/body/CiZmt1cnAOLo81fbxyb0f.gif' },
        { id: 'g-beamknight',   name: 'Golden Beam Knight',   map: 'Tarkan 2',      qty: 1, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/231/body/I0Pl_06EbF_cg8_xqpiJv.gif' },
        // Aida 1-2
        { id: 'g-stonegolem',   name: 'Golden Stone Golem',   map: 'Aida 1 - 2',    qty: 2, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/118/body/zby8OJDOrkss5vlqAqMaG.gif' },
        { id: 'g-witchqueen',   name: 'Golden Witch Queen',   map: 'Aida 1 - 2',    qty: 1, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/232/body/YfpJ4AIjzUdepO9UV-_yw.gif' },
        // Crimson Icarus
        { id: 'g-crust',        name: 'Golden Crust',         map: 'Crimson Icarus', qty: 2, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/119/body/vd1Mkgx6fRmwd5RP59-K_.gif' },
        { id: 'g-phantomknight', name: 'Golden Phantom Knight', map: 'Crimson Icarus', qty: 2, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/233/body/Dubbb1gkJDKI1JlBr8Rgt.gif' },
        // Kanturu / Kanturu Island
        { id: 'g-satyros',      name: 'Golden Satyros',       map: 'Kanturu, Kanturu Island', qty: 2, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/120/body/EePRGPrvir2UQ1uHYrpEv.gif' },
        { id: 'g-bladehunter',  name: 'Golden Blade Hunter',  map: 'Kanturu, Kanturu Island', qty: 2, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/188/body/sRxt2yEXcrbG4jndtnG4M.gif' },
        { id: 'g-phantomknight-k', name: 'Golden Phantom Knight', map: 'Kanturu, Kanturu Island', qty: 2, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/189/body/LFCyTjabmU_1pUJ8SGJ3j.gif' },
        { id: 'g-kentauros',    name: 'Golden Kentauros',     map: 'Kanturu, Kanturu Island', qty: 1, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/121/body/OURA3JmlKzBQ5zdNYHFZy.gif' },
        { id: 'g-gigantis',     name: 'Golden Gigantis',      map: 'Kanturu, Kanturu Island', qty: 1, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/190/body/qOBEIn2d81sYw069IYjLM.gif' },
        { id: 'g-genocider',    name: 'Golden Genocider',     map: 'Kanturu, Kanturu Island', qty: 1, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/191/body/bNlqVNwnnj4uGZeAIi5j5.gif' },
        // Multi-map / Special
        { id: 'g-derkon',       name: 'Golden Derkon',        map: 'Crimson Icarus, Tarkan 1-2, Aida 1-2', qty: 6, color: '#f5a623', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/135/body/sT2bRr2upr-SX8Q8sZZo4.gif', note: '2 por localização' },
        { id: 'g-greatdragon',  name: 'Great Golden Dragon',  map: 'Shadow Abyss',  qty: 1, color: '#ff8800', img: 'https://dreamassets.fra1.digitaloceanspaces.com/images/124/body/9JWBOWm4PENjYWuTBfHTa.gif', note: 'RARO · Drop especial 4x' }
    ]
};
